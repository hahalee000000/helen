"""Tests for helen CLI — run command."""

import os
import tempfile

from helen.cli.__main__ import run_command


class TestRunCommandSuccess:
    """Test helen run with valid programs."""
    ...
    def test_run_file_not_found(self):
        """helen run with nonexistent file returns 1."""
        result = run_command("/nonexistent/path/file.helen")
        assert result == 1

    def test_run_empty_file(self):
        """helen run with empty file returns 0 (nothing to execute)."""
        with tempfile.NamedTemporaryFile(mode="w", suffix=".helen", delete=False) as f:
            f.write("")
            f.flush()
            try:
...
class TestRunCommandSyntaxError:
    """Test helen run with syntax errors."""

    def test_run_syntax_error_returns_1(self):
        """helen run with syntax error returns 1."""
...
class TestRunCommandSemanticError:
    """Test helen run with semantic errors."""

    def test_run_undeclared_variable_returns_2(self):
        """helen run with undeclared variable returns 2."""
