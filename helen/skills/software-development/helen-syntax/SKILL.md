...
author: Helen Team
license: MIT
tags: [helen, syntax, reference, language, chinese-punctuation, chinese-quotes, spawn, channel, v1.18]
---
...
| `llm` | `大模型` | LLM 操作 |
| `act` | `执行` | 自主执行 |
| `spawn` | `分生` | 分生 Agent 并返回 Channel (v1.18) |
| `prompt` | `提示` | 系统提示 |
...
- `if` — LLM 路由（分类分支）
- `spawn` — 分生 Agent 并返回 Channel (v1.18)
...
### spawn 并发 (v1.18)
```helen
// spawn 返回 Channel（邮箱），用于接收 agent 返回值
let mailbox = spawn Worker("task data")

// 接收结果（阻塞直到 agent 完成）
let result = mailbox.receive()

// 多个 agent 并发
let m1 = spawn Worker("task1")
let m2 = spawn Worker("task2")
let m3 = spawn Worker("task3")
...
main {
    let mailbox = spawn Worker(Counter)
    let result = mailbox.receive()
}
```
...
// 协议实现（鸭子类型 — 任何具有 to_string 方法的值都可实现）
// 在 Helen 中，复合数据用 map 表示
let point = {"x": 3, "y": 4}

fn point_to_string(p: map): str {
...
```helen
// spawn 替代了旧式 async/await
let m1 = spawn Task1("First")
let m2 = spawn Task2("Second")
let m3 = spawn Task3("Third")
...
同时新增 Channel 消息队列类型——由 `spawn` 返回，用于 agent 间消息传递。
...
// v1.18: Channel 消息队列（由 spawn 返回）
let mailbox = spawn Worker("data")
...
| 特性 | Shared Store (shared store / 仓库 / 通道) | Channel (spawn 返回) |
|------|------------------------------------------|--------------------------|
| 声明 | `shared store Name { ... }` | 由 `spawn` 返回 |
