"""Semantic analyzer for the Hellen language.
...
class SemanticAnalyzer(Visitor[None]):
    ...
    def _register_stdlib(self) -> None:
        ...
        from hellen.semantic.symbols import Symbol  # noqa: PLC0415
    ...
    def _type_from_typenode(self, type_node: TypeNode | None) -> AnyType | LiteralType | UnionType | OptionalType | NullType:
        ...
        if name == "any":
            return AnyType()
        if name == "list":
            from hellen.semantic.types import ListType
            return ListType(AnyType())
        if name == "map":
            from hellen.semantic.types import MapType
        ...
        if isinstance(node.callee, VariableNode):
            ...
            if callee_name in self._agent_names:
                ...
                if hasattr(agent_node, "params"):
                    ...
                    # Soft check: required params without defaults — Hellen allows partial args
