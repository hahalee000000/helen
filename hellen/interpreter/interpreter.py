"""Interpreter for the Hellen language.
...
from hellen.runtime.llm_runtime import LLMRuntime, MockLLMRuntime
from hellen.runtime.import_resolver import ImportResolver
from hellen.runtime.history import HistoryManager, Message as HistoryMessage
from hellen.interpreter.task import Task, AggregateError
...
class Interpreter(Visitor[object]):
    """AST visitor that executes Hellen programs.
    ...
    def visit_for_stmt(self, node: ForStmtNode) -> object:
        ...
        for item in iterable:
            # Create a new scope for each iteration? No -- Hellen uses
    ...
    def visit_try_stmt(self, node: TryStmtNode) -> object:
        ...
        Execution flow:
        1. Execute try body
        2. If HellenRuntimeError raised, match typed catches in order
        ...
        result = None
        caught = False
        exc_to_rethrow: HellenRuntimeError | None = None
        ...
        try:
            ...
            try:
                ...
                self.environment = old_env
        except HellenRuntimeError as exc:
            caught = True
            exc_to_rethrow = exc
    ...
    def visit_import_stmt(self, node: ImportStmtNode) -> object:
        ...
        Supported formats:
        - .hellen: Parse and register agents/functions to global namespace
    ...
    def _truthy(value: object) -> bool:
        """Convert a Hellen value to boolean.
    ...
    def _equal(a: object, b: object) -> bool:
        """Check equality between two Hellen values."""
    ...
    def _stringify(value: object) -> str:
        """Convert a Hellen value to its string representation."""
        if value is None:
            return "null"
        if isinstance(value, bool):
