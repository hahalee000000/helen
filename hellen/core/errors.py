"""Error types, warnings, and an error reporter for the Hellen language.

Defines the ``ErrorCode`` enumeration, ``HellenError`` / ``HellenWarning``
...
class ErrorCode(Enum):
    """Numeric codes for Hellen compilation diagnostics.
...
@dataclass
class HellenError(Exception):
    """A recoverable or fatal error encountered during Hellen compilation.
...
@dataclass
class HellenWarning:
    """A non-fatal warning encountered during Hellen compilation.
...
class ErrorReporter:
    ...
    _errors: list[HellenError] = field(default_factory=list)
    _warnings: list[HellenWarning] = field(default_factory=list)
    ...
    def error(
        ...
        """
        self._errors.append(HellenError(code, message, span))
    ...
    def warning(
        ...
        Args:
            ...
            span: Optional source location.
        """
        self._warnings.append(HellenWarning(code, message, span))

    @property
    def errors(self) -> list[HellenError]:
        """Return a copy of the collected errors.

        Returns:
            A new list containing all ``HellenError`` instances recorded
            so far.
        """
        return list(self._errors)

    @property
    def warnings(self) -> list[HellenWarning]:
        """Return a copy of the collected warnings.

        Returns:
            A new list containing all ``HellenWarning`` instances recorded
            so far.
        """
