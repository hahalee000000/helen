"""LLM runtime interface and mock implementation for the Hellen language.
...
class LLMRuntime(ABC):
    """Abstract interface for LLM operations in Hellen.

    Three core methods map to Hellen's llm statements:
    - route()  → llm if
    - choose() → llm choose
    - act()    → llm act
    """
