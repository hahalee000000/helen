"""Import Resolver for the Hellen language (HLD 3.9 M8).

Resolves and loads imported modules in multiple formats:
- .hellen: Parse recursively, register agents/functions (don't execute main)
...
class ImportResult:
    ...
    Attributes:
        ...
        content: Parsed/loaded content (ProgramNode for .hellen, str for text,
                 dict/list for json/yaml).
        format: The detected file format ('hellen', 'text', 'json', 'yaml').
    """
...
class ImportResolver:
    """Resolve and load imports for Hellen programs (HLD 3.9).
    ...
    def resolve(
        ...
        if abs_resolved in self._loaded:
            ...
            if alias in self._data:
                return ImportResult(
                    path=abs_resolved, content=self._data[alias], format="hellen"
                )
            return None
        ...
        alias = os.path.splitext(os.path.basename(resolved))[0]
        if fmt == "hellen":
            self._register_hellen(content, alias)
        else:
    ...
    def _load_file(self, path: str, fmt: str) -> Any:
        ...
        if fmt == "hellen":
            return self._parse_hellen(raw, path)
        if fmt == "json":
    ...
    def _parse_hellen(self, source: str, filename: str) -> ProgramNode:
        """Parse a .hellen source file into a ProgramNode."""
        scanner = Scanner(source=source, file=filename)
        tokens = scanner.scan_all()
        parser = Parser(tokens)
        return parser.parse()

    def _register_hellen(self, program: ProgramNode, alias: str) -> None:
        """Register agents/functions from a parsed .hellen file (HLD 3.9.1).
        ...
        for stmt in program.statements:
            ...
            from hellen.core.ast import (  # noqa: PLC0415
                AgentDeclNode,
                FunctionDeclNode,
            )
