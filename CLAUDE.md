...
- **spawn resume (v1.27)**: Optional `spawn Agent(...) resume("<session_id>")` clause (Chinese alias `恢复会话(...)`) makes the spawned agent **continue a previously saved child-session transcript** (true resumption - appends to that session's transcript) instead of starting fresh. `resume` is an identifier clause (not a keyword), so the bilingual keyword count stays at 89. A non-existent session_id creates a fresh session with that id (graceful fallback). Distinct from `resume_session(sid)` (which imports another session's messages into a new session). Resuming restores LLM conversation memory (the prior exchange is visible to `llm act` inside agent `main {}` via `expose_resumed_messages_to` at invocation entry), not runtime variable state; new messages created during the run still respect per-invocation isolation. A cross-process lock (`SessionManager.acquire_session_lock`, PID file at `<session_dir>/<session_id>/session.lock`) prevents two processes from corrupting the same transcript; stale locks and same-process reuse are auto-reclaimed.
