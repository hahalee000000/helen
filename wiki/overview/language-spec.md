...
| `spawnagent` | `生成` | 并发 agent 生成 (v1.18) |
| `prompt` | `提示` | 系统提示 |
...
| `spawnagent` | 并发生成 agent (v1.18) | `let m = spawnagent Worker("task")` |
...
**v1.12**: 新增 `STORE`。**v1.14**: 删除 `STREAM`。**v1.18**: 删除 `ASYNC`/`AWAIT`/`DETACH`/`CHANNEL`，新增 `SPAWNAGENT`。
...
### 表达式节点 (15)
`LiteralNode` `VariableNode` `BinaryOpNode` `UnaryOpNode` `GroupingNode` `CallNode` `CallArgNode` `AccessNode` `IndexNode` `ListLiteralNode` `MapLiteralNode` `MapEntryNode` `PipeExprNode` `MatchExprNode` `SpawnagentExprNode`

**v1.18**: 新增 `SpawnagentExprNode`。删除 `AsyncCallExprNode`。
...
v1.18 起 `act_async()` / `act_stream_async()` 已删除，由 `spawnagent` + Channel 替代。
...
v1.18 起 `channel X { fields }` 声明语法已删除。Channel 现在通过 `Channel()` 构造函数或 `spawnagent` 创建，是消息队列（send/receive）。

```helen
// v1.18 新语法：通过 spawnagent 创建 channel
let mailbox = spawnagent Worker("task")
...
### v1.18: spawnagent 并发原语

**Breaking change**: `async`/`await`/`detach` 关键字及 `channel X { fields }` 声明语法已删除，由 `spawnagent` + Channel 消息队列替代。
...
- 新增：`SpawnagentExprNode`、`channel.py`、`mailbox.py`
...
- [[interpreter/spawnagent|并发与 spawnagent]]
