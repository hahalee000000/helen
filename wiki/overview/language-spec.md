...
> 版本: v1.14 | 关键字: 92 (46 英文 + 46 中文) | Token 类型: 86 | AST 节点: 63 | Visitor 方法: 57
...
### 声明节点 (5)
`AgentDeclNode` `SharedStoreDeclNode` `ChannelDeclNode` `TypeNode` `OptionalTypeNode` `UnionTypeNode` `LiteralTypeNode`

**v1.12**: 新增 `SharedStoreDeclNode`。**v1.13**: 新增 `ChannelDeclNode`。`AgentDeclNode` 增加 `isolation_level` 字段。
