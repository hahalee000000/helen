...
declaration   → decorator? (agent_decl | fn_decl | import_stmt | shared_store_decl | channel_decl)
...
### Shared Store 与 Channel (v1.12 / v1.13)
...
- `channel`: agent 间通信端点（类型安全）
- 运行时均复用 `SharedStore` 类（RLock 线程安全）
