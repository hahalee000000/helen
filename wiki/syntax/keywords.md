...
> 89 关键字 (44.5 英文 + 44.5 中文) | 按功能分类 | 另见 `@` 隔离注解运算符 (v1.12)
...
**v1.18 变更**：`channel X { fields }` 声明语法已删除。Channel 现在通过 `Channel()` 构造函数或 `spawnagent` 创建，是消息队列（send/receive），不再是带锁结构体。需要带锁结构体时使用 `shared store`。

**注意**：`通道` 仍可作为 `SharedStore` 的中文别名使用，但不再有关键字级别的 `channel` 声明语法。
