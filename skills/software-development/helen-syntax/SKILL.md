...
author: Helen Team
license: MIT
tags: [helen, syntax, reference, language, chinese-punctuation, chinese-quotes, spawnagent, channel, v1.18]
---

# Helen 语法参考

## 关键字（89 个：44.5 英文 + 44.5 中文）
...
| `llm` | `大模型` | LLM 操作 |
| `act` | `执行` | 自主执行 |
| `spawnagent` | `生成` | 生成 Agent 并返回 Channel (v1.18) |
| `prompt` | `提示` | 系统提示 |
...
agent SmartAssistant {
    context {
        compression "graduated"      // 压缩策略: "none" / "graduated" / "traditional"
        cache-aware true             // 缓存感知压缩
        working-memory true          // 自动跟踪文件、决策、错误
...
### LLM 语句
- `llm` — LLM 操作关键字
- `act` — 自主执行（调用 LLM，支持 `on_chunk`/`on_complete` 回调）
- `if` — LLM 路由（分类分支）
- `spawnagent` — 生成 Agent 并返回 Channel (v1.18)
...
### spawnagent 并发 (v1.18)
```helen
// spawnagent 返回 Channel（邮箱），用于接收 agent 返回值
let mailbox = spawnagent Worker("task data")

// 接收结果（阻塞直到 agent 完成）
let result = mailbox.receive()

// 多个 agent 并发
let m1 = spawnagent Worker("task1")
let m2 = spawnagent Worker("task2")
let m3 = spawnagent Worker("task3")
...
// 多 Channel 选择（第一个就绪的胜出）
let result = mailbox_select([m1, m2, m3])

// Channel 方法
mailbox.send("消息")        // 发送消息
mailbox.receive()           // 阻塞接收
mailbox.try_receive()       // 非阻塞接收（无消息返回 null）
mailbox.cancel()            // 取消等待中的 agent
mailbox.close()             // 关闭 channel
...
main {
    let mailbox = spawnagent Worker(Counter)
    let result = mailbox.receive()
}
```
...
### 5. spawnagent 并发 (v1.18)

```helen
// spawnagent 替代了旧式 async/await
let m1 = spawnagent Task1("First")
let m2 = spawnagent Task2("Second")
let m3 = spawnagent Task3("Third")
...
// 多 Channel 选择
let first = mailbox_select([m1, m2, m3])
```

**并发性能**: HTTP 层使用 httpx 连接池，并发请求天然高效。
...
### 3. Channel 通道（v1.13 / v1.18 更新）

**v1.13**: `通道` 是 `shared store` 的中文别名，声明语法完全一致。

**v1.18**: `channel X { fields }` 声明语法已移除。`通道` 仅作为 `shared store` 的中文别名保留。
同时新增 Channel 消息队列类型——由 `spawnagent` 返回，用于 agent 间消息传递。

```helen
// ✅ v1.18: 通道 = shared store 的中文别名
shared store 计数器 {
  let count: int = 0
  fn increment() { count = count + 1 }
  fn get(): int { return count }
...
// v1.18: Channel 消息队列（由 spawnagent 返回）
let mailbox = spawnagent Worker("data")
mailbox.send("hello")           // 发送消息
let msg = mailbox.receive()     // 阻塞接收
let maybe = mailbox.try_receive() // 非阻塞

// 多 Channel 选择
let ready = mailbox_select([m1, m2, m3])
```

**Channel 消息队列 vs Shared Store**:

| 特性 | Shared Store (shared store / 仓库 / 通道) | Channel (spawnagent 返回) |
|------|------------------------------------------|--------------------------|
| 声明 | `shared store Name { ... }` | 由 `spawnagent` 返回 |
| 语义 | 共享可变状态（shared-memory） | Agent 间消息传递（message-passing） |
| 访问方式 | 方法调用 | send/receive/try_receive |
| 中文别名 | `共享 仓库` 或 `通道` | 无（运行时类型） |
